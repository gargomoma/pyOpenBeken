...
def index():
    global list_ips,devices,release,devmgr
    ...
    return render_template('home_table.html', data=devmgr.devices
                               , list_ips=list_ips , release= release, message = msg)
...
def find_devices():
    global devmgr,release, list_ips, msg
    network.get_network_ips(ip_chunks_size=10)
    network.obkn_api_scan(ip_chunks_size=2)
...
def refresh_releases():
    global devmgr,release
    devmgr.check_ota(redownload_release=True)
    release = devmgr.releaseMngr.latest
    return redirect('/')
...
def run_ota_devices():
    ...
    return f"{'; '.join(devices_to_update)} board(s) will be updated.<br>Please wait a few minutes.<a href='/'>Return to main page<//a>"
